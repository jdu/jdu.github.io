...
EXTRA_PX = {
    tag: (top * BASE_FONT_PX, bottom * BASE_FONT_PX)
    for tag, (top, bottom) in EXTRA_REM.items()
}
...
CODE_BLOCK_EXTRA_PX = (
    1.5 * BASE_FONT_PX + 1.0 * BASE_FONT_PX
        ...
        def repl(m):
            ...
            if kind == "ref":
                ...
                if entry["kind"] == "sidenote":
                    ...
                    placeholders[token] = (
                        f'<sup class="sidenote-ref">{entry["number"]}</sup>'
                    )
...
def render_list_html(tag, entries):
    ...
    for e in entries:
        sub_html = (
            render_list_html(e["sub"]["tag"], e["sub"]["entries"]) if e["sub"] else ""
        )
...
def list_group_height(tag, entries, font_px, width_px, item_extra_px):
    ...
    for e in entries:
        ...
        if e["sub"]:
            total += list_group_height(
                e["sub"]["tag"], e["sub"]["entries"], font_px, width_px, item_extra_px
            )
    return total
    ...
    for b in sub_blocks:
        ...
        if tag == "image":
            ...
            total += list_group_height(
                ...
                MARGINNOTE_FONT_PX,
                MARGIN_WIDTH_PX,
                LI_ITEM_EXTRA_PX * 0.8,
            )
        elif tag == "code-block":
            total += (
                CODE_BLOCK_EXTRA_PX + b["line_count"] * CODE_FONT_PX * CODE_LINE_HEIGHT
            )
        elif tag == "table":
            total += TABLE_EXTRA_PX + b["row_count"] * (
                TABLE_FONT_PX * LINE_HEIGHT + TABLE_ROW_PADDING_PX
            )
        else:
            lines = text_lines(
                b.get("text_len", 0), MARGINNOTE_FONT_PX, MARGIN_WIDTH_PX
            )
...
def block_height_px(block, is_chapter_start=False):
    ...
    if tag == "code-block":
        return (
            CODE_BLOCK_EXTRA_PX + block["line_count"] * CODE_FONT_PX * CODE_LINE_HEIGHT
        )

    if tag == "table":
        return TABLE_EXTRA_PX + block["row_count"] * (
            TABLE_FONT_PX * LINE_HEIGHT + TABLE_ROW_PADDING_PX
        )

    if tag == "chapter-title" and is_chapter_start:
        lines = text_lines(
            block["text_len"], FONT_PX["chapter-title"], CONTENT_WIDTH_PX
        )
        return (
            CHAPTER_TITLE_MARGIN_TOP_PX
            + lines * FONT_PX["chapter-title"] * LINE_HEIGHT
            + CHAPTER_TITLE_GAP_PX
        )
    ...
    if tag in ("ul", "ol"):
        return list_group_height(
            tag, block["entries"], FONT_PX["li"], MAIN_WIDTH_PX, LI_ITEM_EXTRA_PX
        )
...
def paginate(blocks, content_height_px):
    ...
    for block in blocks:
        ...
        if starts_chapter:
            page_is_bare_part_header = (
                len(current) == 1 and current[0]["tag"] == "part-header"
            )
        ...
        if current and (
            next_main > content_height_px or next_margin > content_height_px
        ):
            pages.append(current)
            current = []
            main_used = 0.0
        ...
        if b["tag"] == "marginnote":
            ...
            content_html_parts.append(
                b["html"].replace(
                    ...
                    f'<div class="marginnote" style="top:{top_px:.1f}px">',
                    1,
                )
            )
        else:
    ...
    while i < n:
        ...
        if stripped == "|===":
            ...
            for row_index, cells in enumerate(rows):
                ...
                cells_html = "".join(
                    f"<{cell_tag}>{apply_inline(c)}</{cell_tag}>" for c in cells
                )
        ...
        if stripped.startswith(">"):
            ...
            while i < n:
                ...
                i += 1
            quote_blocks = convert_body(
                quote_lines, heading_base, seen_ids, apply_lede=False
            )
            blocks.append(
                {
                    "tag": "blockquote",
        ...
        if list_match:
            ...
            list_stack[-1][2].append(
                {
                    "text_len": len(item_text),
                    "html": apply_inline(item_text),
                    "sub": None,
                }
            )
            i += 1
        ...
        admon_match = re.match(
            r"^(NOTE|TIP|IMPORTANT|WARNING|CAUTION):\s*(.*)$", stripped
        )
        ...
        placeholder = [
            {
                "tag": "p",
                "text_len": 0,
                "html": f'<p class="missing">[missing file: {html.escape(path.name)}]</p>',
            }
        ]
        ...
        blocks = [
            {
                "tag": "p",
                "text_len": 0,
                "html": '<p class="missing">[chapter not yet written]</p>',
            }
        ]
    return title, blocks
...
def estimate_toc_part_label_height(part_label_text):
    ...
    return (
        lines * font_px * LINE_HEIGHT + 0.5 * BASE_FONT_PX
    )  # .toc-part-label margin-bottom
...
def estimate_toc_chapter_height(chapter_label, subheading_titles):
    ...
    height = (
        lines * font_px * LINE_HEIGHT + 1.0 * BASE_FONT_PX
    )  # .toc-chapter margin-bottom
    ...
    for sub_title in subheading_titles:
        ...
        height += (
            sub_lines * font_px * LINE_HEIGHT + 0.2 * BASE_FONT_PX
        )  # .toc-subheadings li padding
    return height
...
def build(index_path: Path, out_path: Path):
    ...
    for part in parts:
        ...
        if is_numbered:
            ...
            summary_html = ""
            if summary_raw:
                summary_blocks = convert_body(
                    summary_raw.splitlines(),
                    heading_base=3,
                    seen_ids=seen_ids,
                    apply_lede=False,
                )
            ...
            page_counter += (
                1  # divider page itself stays unnumbered, but still occupies a page
            )
        ...
        parts_html.append(
            f'<section class="part-group" id="{part_id}">{divider_html}{sheets_html}</section>'
        )
        ...
        toc_entries.append(
            (part_label_html, estimate_toc_part_label_height(part_label_text))
        )
        ...
        for info in chapters_info:
            ...
            subheadings_html = (
                f'<ol class="toc-subheadings">{"".join(subheading_lines)}</ol>'
                if subheading_lines
                else ""
            )
            ...
            toc_entries.append(
                (
                    chapter_html,
                    estimate_toc_chapter_height(chapter_label, subheading_titles),
                )
            )
    ...
    copyright_html = ""
    if copyright_text:
        copyright_blocks = convert_body(
            copyright_text.splitlines(),
            heading_base=3,
            seen_ids=seen_ids,
            apply_lede=False,
        )
        copyright_html = (
            '<div class="sheet copyright-page">'
            + "".join(b["html"] for b in copyright_blocks)
            + "</div>"
        )
    ...
    dedication_html = ""
    if dedication_text:
        dedication_blocks = convert_body(
            dedication_text.splitlines(),
            heading_base=3,
            seen_ids=seen_ids,
            apply_lede=False,
        )
        dedication_html = (
            '<div class="sheet dedication">'
            + "".join(b["html"] for b in dedication_blocks)
            + "</div>"
        )
    ...
    if acknowledgements_text:
        ...
        ack_flow.extend(
            convert_body(
                acknowledgements_text.splitlines(),
                heading_base=3,
                seen_ids=seen_ids,
                apply_lede=False,
            )
        )
        acknowledgements_html = "".join(
            render_sheet(p) for p in paginate(ack_flow, CONTENT_HEIGHT_PX)
        )
    ...
    if preface_path.exists():
        ...
        preface_html = "".join(
            render_sheet(p) for p in paginate(preface_flow, CONTENT_HEIGHT_PX)
        )
    ...
    if bio_path.exists():
        ...
        bio_html = "".join(
            render_sheet(p) for p in paginate(bio_flow, CONTENT_HEIGHT_PX)
        )

    front_matter_html = (
        dedication_html + acknowledgements_html + preface_html + bio_html
    )
            ...
            rr_flow.append(
                {
                    "tag": "ul",
                    "entries": rr_entries,
                    "html": render_list_html("ul", rr_entries),
                }
            )
    ...
    if bibliography:
        ...
        bib_flow = [
            ...
            },
            {
                "tag": "ul",
                "entries": bib_entries,
                "html": render_list_html("ul", bib_entries),
            },
        ]
        bib_sheets = []
    ...
    if index_entries:
        ...
        for term in sorted_terms:
            ...
            lines = text_lines(
                len(term) + len(pages_str), idx_font_px, TOC_COLUMN_WIDTH_PX
            )
            idx_entries_for_pagination.append(
                (term_html, lines * idx_font_px * LINE_HEIGHT + 0.2 * BASE_FONT_PX)
            )

        idx_sheets = []
        for i, page_items in enumerate(
            paginate_two_column(
                idx_entries_for_pagination, CONTENT_HEIGHT_PX, TOC_HEADING_HEIGHT_PX
            )
        ):
            classes = "sheet toc-page" + (" chapter-start" if i == 0 else "")
            heading = (
                '<h3 class="chapter-title" id="index">Index</h3>' if i == 0 else ""
            )
            idx_sheets.append(
...
def watch(index_path: Path, out_path: Path, interval: float = 0.5):
    print(
        f"Watching {index_path.name} and its chapters for changes (Ctrl+C to stop)..."
    )
...
def main():
    ...
    parser.add_argument(
        "--index", default="_index.yaml", help="Path to the book's YAML structure file"
    )
    parser.add_argument("--out", default="book.html", help="Output HTML file path")
    parser.add_argument(
        "--watch",
        action="store_true",
        help="Rebuild automatically whenever the index or a chapter file changes",
    )
    parser.add_argument(
        "--interval",
        type=float,
